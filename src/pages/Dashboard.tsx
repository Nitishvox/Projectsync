  ...
  const fetchDashboardData = async () => {
    ...
    try {
      ...
      const [statsRes, projRes, taskRes, logsRes] = await Promise.all([
        api.get('/dashboard'),
        api.get('/projects?page=1&limit=4'),
              ...
              <div className="space-y-2.5">
                {recentTasks.map((task) => {
                  ...
                  return (
                    <div
                      key={task.id}
                      className={`p-3 rounded-lg border border-gray-100 dark:border-slate-800 flex items-center justify-between gap-3 transition-colors ${isDone
                    ...
                    >
                      <div className="flex items-center gap-2.5 min-w-0">
                        ...
                        <span
                          className={`text-xs font-medium truncate ${isDone ? 'line-through text-gray-400 dark:text-slate-500' : 'text-gray-900 dark:text-slate-200'
                            }`}
                        >
                          {task.name}
                        </span>
                      ...
                      <span
                        className={`text-[10px] font-semibold px-2 py-0.5 rounded-full flex-shrink-0 ${task.priority === 'HIGH'
              ...
              return (
                <div key={log.id} className="p-3.5 px-5 flex items-center justify-between gap-3 text-xs hover:bg-gray-50/60 dark:hover:bg-slate-800/50 transition-colors">
                  <div className="flex items-center gap-3 min-w-0">
                    <span
                      className={`px-2 py-0.5 rounded-md font-semibold text-[10px] tracking-wide uppercase ${isProject
